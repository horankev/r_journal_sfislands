...
For R users, the \CRANpkg{spdep} package (Bivand et al. 2005) has long been popular for the
creation of these matrices. More recently, with the increasing use of \CRANpkg{sf}
structures and their compatibility with the \CRANpkg{tidyverse} (Wickham et al. 2019), the \CRANpkg{sfdep} (Pebesma 2018) package (Parry and Locke 2024) has presented the same
...
are for use with the \CRANpkg{mgcv} package (Wood 2011). It is straightforward to use
\texttt{sfislands} neighbourhood structures in models with \CRANpkg{mgcv} but it can be quite awkward
...
\item
  ...
  \item
    these maps are created using \CRANpkg{ggplot2} (Wickham 2016), which allows users to apply the desired styling and themes using \CRANpkg{ggplot2} syntax,
  \item
...
\pkg{r-inla}, \pkg{rstan} (Stan Development Team 2020) or \CRANpkg{mgcv}. Rather than having a separate neighbours
...
estimates from \CRANpkg{mgcv} model. \texttt{sfislands} has two post-functions to conveniently extract and visualise these.
...
Use \texttt{st\_augment()} to get estimates from \CRANpkg{mgcv} model in format
similar to that used by the \CRANpkg{broom} package. For instance, we see from the code below that the original dataframe is now augmented with columns for ``mrf.smooth.province'' and ``mrf.smooth.province'' which shows the estimate and standard errors for the \(\gamma_i\) component. Note that this is how we would expect them to be named, based on the previous discussion surrounding Table \ref{tab:staugtab-latex}.
...
Use \texttt{st\_augment()} and \texttt{st\_quickmap\_preds} in the following code to get a quick visualisation of
...
\texttt{sfislands} makes it easier to take account of this situation. We can look
...
\item
  \texttt{st\_knn()} from \CRANpkg{sfdep}.
\end{itemize}
