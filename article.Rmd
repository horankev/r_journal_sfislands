...
creation of these matrices. More recently, with the increasing use of \CRANpkg{sf} structures  [@sf] and their compatibility with the \CRANpkg{tidyverse}  [@tidyverse], the \CRANpkg{sfdep} package [@sfdep] has presented the same
...
is shown in Figure \@ref(fig:`r ifelse(knitr::is_html_output(), 'ind-contig1html', 'ind-contig1')`). Note how is can be styled with a combination of internal arguments (size, colour, fill etc.) and additional \CRANpkg{ggplot2} layers.
...
There are no islands or discontiguities in this geometry of Liverpool so using `st_bridges()` will give the same structure as would `sfdep::st_contiguity()` (see Figure \@ref(fig:liv-contigs), with a magnified section for visibility purposes, created using \pkg{ggmagnify}  [@ggmagnify]).
...
# read from github
liverpool_hmodel <- readRDS(gzcon(url("https://github.com/horankev/r_journal_sfislands/raw/main/data/liverpool_model2.rds")))
...
# read from github
liverpool_hsamodel <- readRDS(gzcon(url("https://github.com/horankev/r_journal_sfislands/raw/main/data/liverpool_hsamodel.rds")))
...
# read from github
liverpool_hsamodel_dist_band <- readRDS(gzcon(url("https://github.com/horankev/r_journal_sfislands/raw/main/data/liverpool_hsamodel_dist_band.rds")))
