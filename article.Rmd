...
creation of these matrices. More recently, with the increasing use of \CRANpkg{sf}  [@sf]
...
how the \CRANpkg{broom} package  [@broom] operates. The `geometry` column, as per convention, remains as the last column of the augmented dataframe, while the predictions are positioned immediately before it. ^[In a similar way, `st_augment()` can also be used to append the random effects from \CRANpkg{lme4}   [@lme4] and \CRANpkg{nlme}   [@nlme] models to an `sf` dataframe, which can then be easily mapped using `st_quickmap_preds()`. Compatability with models created using different packages can be introduced in the future.] The spatially varying predictions which `st_augment()` extracts from an \CRANpkg{mgcv} model are 
...
visual representation of the structure. ^[`st_quickmap_nb()` can also be used to visualise any contiguity structure created by `spdep` or `sfdep` as long as that structure is included in an `sf` dataframe as a column named `nb`.] Because this map is created using \CRANpkg{ggplot2}, it can be easily supplemented by adding a layer showing the course of the river Thames which is also visible in Figure \@ref(fig:lon-contig1).
